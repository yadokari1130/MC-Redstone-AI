...
description: Redstone MCP Server全体の仕様書の目次と概要（各章の詳細はspec-docsディレクトリを参照）
---

# Redstone MCP Server 詳細仕様書

本仕様書は、Redstone MCP Serverの開発に必要な詳細要件を各章ごとに定義したものです。
...
5. [MCPサーバー (AI Tools) 仕様](./spec-docs/05-mcp-tools.md)
   - AIが利用するToolの定義 (`get_blocks`, `place_blocks`, `interact_block`)
