...
3. [機能要件詳細](./spec-docs/03-functional-requirements.md)
   - 情報取得、ブロック配置、エンティティ操作、インタラクト操作の実現要件
4. [Minecraftプラグイン APIインターフェース仕様](./spec-docs/04-api-specification.md)
   - プラグイン側のHTTPエンドポイント定義 (`GET /api/blocks`, `POST /api/blocks`, `POST /api/interact`, `POST /api/kill-entities`)
5. [CLIコマンド (AI用) 仕様](./spec-docs/05-cli-commands.md)
   - AIが利用するコマンドの定義 (`get_blocks`, `place_blocks`, `interact_block`, `kill_entities`)
6. [データモデル・JSON構造](./spec-docs/06-data-model.md)
   - 送受信されるブロック・エンティティデータスキーマの定義 (`BlockData`, `EntityData`)
