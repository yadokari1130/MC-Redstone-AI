...
### 3. MCPサーバー (Python)
AIエージェントとMinecraftプラグイン（HTTPサーバー）の間に立ち、中継および制御を行うMCP (Model Context Protocol) サーバーです。AIはここから提供される操作用Tool (`get_blocks`, `place_blocks`, `interact_block`) を駆使して、安全かつ正確に環境を操作します。
...
- [総合仕様書・目次](.agents/rules/specification.md)
  ...
  - 5. [MCPサーバー (AI Tools) 仕様](.agents/rules/spec-docs/05-mcp-tools.md)
